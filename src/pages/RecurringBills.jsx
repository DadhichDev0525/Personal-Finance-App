...
import { sortingOptions } from "../dropdownOptions";
...
const RecurringBills = () => {
  ...
  const totalBill = recurringTransactions.reduce(
    (acc, tx) => acc + Math.abs(tx.amount),
    0
  );
  useEffect(() => {
    let updatedList = [...recurringTransactions];
    if (val.trim() !== "") {
    ...
    updatedList.sort((a, b) => {
      const A = Math.abs(a.amount)
      const B = Math.abs(b.amount)
      switch (sortOption) {
        case "Latest":
  ...
  }, [recurringTransactions, val, sortOption]);
    ...
    recurringTransactions.reduce(
      (acc, tx) => {
        ...
        const differenceInDays = Math.ceil(
          (date - today.getTime()) / (1000 * 3600 * 24)
        );
        ...
        <Panel className="flex-1 min-w-[265px]">
          <div className="py-4 flex flex-col md:flex-row justify-between md:items-center">
            <input
              ...
              className="border border-gray-500 rounded-md p-2 focus:outline-0 max-w-lg md:w-[40%]"
            />
              ...
              <thead className=" border-b border-gray-400 text-sm text-gray-600">
                <tr>
                  ...
                  <th className=" p-3 flex items-center justify-center">
                    <p className="w-[120px] flex">Due Date</p>
                  </th>
              ...
              <tbody>
                {!paginatedBills || paginatedBills.length === 0 ? (
                  ...
                  paginatedBills.map((bill) => {
                    ...
                    const dueSoon =
                      differenceInDays >= 0 && differenceInDays < 4;
                    return (
                      <tr
                        key={bill.id}
